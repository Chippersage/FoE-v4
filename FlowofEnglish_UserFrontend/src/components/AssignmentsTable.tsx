...
sx={{ fontWeight: 600, color: "text.primary", lineHeight: 1.3, maxHeight: "2.6em", overflow: "hidden", textOverflow: "ellipsis", display: "-webkit-box",
WebkitLineClamp: 2, WebkitBoxOrient: "vertical", wordBreak: "break-word", }}  data-tour-id="topic"
>
...
<TableCell sx={{ minWidth: 40 }}>
  <TextField type="number" size="small" value={localScore} onChange={handleLocalScoreChange}
    error={fieldErrors?.score} helperText={fieldErrors?.score ? "Score is required" : ""} disabled={isDisabled}
    inputProps={{ min:0, max:assignment.subconcept.subconceptMaxscore, style:{ width:"36px", textAlign:"center" } }} />
</TableCell>
...
<TableCell sx={{ minWidth: 180, maxWidth: 180 }}>
  <Box sx={{ position: "relative" }}>
    <TextField fullWidth size="small" multiline maxRows={3}
      value={localRemarks} onChange={handleLocalRemarksChange}
      error={fieldErrors?.remarks}
      helperText={fieldErrors?.remarks ? "Remark is required" : ""}
      disabled={isDisabled}
      inputProps={{ maxLength: 150, style:{ paddingBottom: "18px", fontSize:"0.8rem" } }}
    />
    <Box sx={{ position:"absolute", bottom:4, right:8, fontSize:"0.7rem", opacity:0.6, pointerEvents:"none" }}>
...
{/* 
  ...
  <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
    ...
    <label htmlFor={`correction-file-${assignment.assignmentId}`}>
      ...
      />
      <Tooltip title="Upload correction file">
        <IconButton size="small" color="primary" component="span" disabled={isDisabled}>
          <CloudUploadIcon fontSize="small" />
        </IconButton>
      </Tooltip>
    </label>

    {/* Filename or View button */}
    {editedAssignments[assignment.assignmentId]?.file?.name ? (
      <Typography
        variant="caption"
        noWrap
        sx={{ maxWidth: "48px", fontSize: "0.65rem" }}
      >
        {editedAssignments[assignment.assignmentId].file.name}
      </Typography>
    ) : (
...
<TableContainer component={Paper} sx={{ mt: 2, width: "100%", overflowX: "auto", overflowY: "hidden", 
    WebkitOverflowScrolling: "touch", boxShadow: "0 2px 8px rgba(0,0,0,0.05)",
    }} >
  <Table size="small"sx={{ width: "auto", minWidth: "100%", tableLayout: "fixed" }}>
{/* <TableHead>
<TableRow sx={{ backgroundColor: "#f0f8ff" }}> {tableHeaders.map((header) =>
(!isMobile || header.mobile) && (
<TableCell key={header.id} align={header.align || "left"}
sx={{ whiteSpace: "nowrap", width: header.width, minWidth: header.width, py: 1, fontSize: '0.75rem', fontWeight: 600, lineHeight: 1.1, textAlign: "center", }} >
{header.id.includes('.') ? (
<TableSortLabel active={orderBy === header.id} direction={orderBy === header.id ? order : "asc"}
onClick={() => handleRequestSort(header.id)}> {header.label} {header.id === "score" || header.id === "remarks" && ( <span style={{ color: "red" }}> *</span>
)}</TableSortLabel> ) : ( <>{header.label} {(header.id === "score" || header.id === "remarks") && (
<span style={{ color: "red" }}> *</span> )} </> )} </TableCell> ) )}
</TableRow>
</TableHead> */}
<TableHead>
...
sx={{ width: header.width, minWidth: header.width, maxWidth: header.width,
py: 1, fontSize: "0.7rem", fontWeight: 600, lineHeight: 1.1, textAlign: "center",
